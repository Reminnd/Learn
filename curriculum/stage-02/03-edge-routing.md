# LangGraph 核心 / Chapter 03

# Edge、Conditional Edge 与 Routing
...
- if/elif → conditional edge
- 手工 router → LangGraph routing
- model route → policy + model hybrid
...
````yaml
chapter_id: stage-02-03-edge-routing
prerequisites: ["stage-02-02-node"]
required_exercises:
  - id: EX1
    task: "围绕Edge、Conditional Edge 与 Routing，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：理解固定边与条件边"
      - "能复现或具体分析工程问题：路由错误"
      - "能解释并验证框架映射：if/elif → conditional edge"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：理解固定边与条件边"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：if/elif → conditional edge"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：路由错误"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
