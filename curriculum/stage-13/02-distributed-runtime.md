...
# Distributed Runtime：Worker、Queue、幂等
...
- sync invoke → job/worker
- local retry → durable retry
...
````yaml
chapter_id: stage-13-02-distributed-runtime
prerequisites: ["stage-13-01-fastapi-storage"]
required_exercises:
  - id: EX1
    task: "围绕Distributed Runtime：Worker、Queue、幂等，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：理解异步任务和分布式 Worker"
      - "能复现或具体分析工程问题：重复执行"
      - "能解释并验证框架映射：sync invoke → job/worker"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：理解异步任务和分布式 Worker"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：sync invoke → job/worker"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：重复执行"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
