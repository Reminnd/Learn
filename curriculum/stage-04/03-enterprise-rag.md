...
- 静态索引 → incremental indexing
- 全局检索 → metadata permission filter
...
````yaml
chapter_id: stage-04-03-enterprise-rag
prerequisites: ["stage-04-02-retrieval-quality"]
required_exercises:
  - id: EX1
    task: "围绕企业 RAG：增量索引、权限、多租户，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：理解文档生命周期"
      - "能复现或具体分析工程问题：文档更新"
      - "能解释并验证框架映射：静态索引 → incremental indexing"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：理解文档生命周期"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：静态索引 → incremental indexing"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：文档更新"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
