...
# 检索质量：Query Rewrite、Hybrid Search、Reranking
...
````yaml
chapter_id: stage-04-02-retrieval-quality
prerequisites: ["stage-04-01-rag-foundation"]
required_exercises:
  - id: EX1
    task: "围绕检索质量：Query Rewrite、Hybrid Search、Reranking，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：理解召回与排序"
      - "能复现或具体分析工程问题：术语不匹配"
      - "能解释并验证框架映射：单 query → rewrite/multi-query"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：理解召回与排序"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：单 query → rewrite/multi-query"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：术语不匹配"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
