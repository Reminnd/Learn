...
- 理解 Deep Agent 为什么需要 planning/subagents/context/workspace
- 与普通 Agent 比较
...
- 已有 LangGraph 能力 → Deep Agents abstraction
- 普通 tool agent → long-horizon agent
...
````yaml
chapter_id: stage-08-01-deep-agents-model
prerequisites: ["stage-07-04-multi-agent-reliability"]
required_exercises:
  - id: EX1
    task: "围绕Deep Agents 心智模型，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：理解 Deep Agent 为什么需要 planning/subagents/context/workspace"
      - "能复现或具体分析工程问题：过度自治"
      - "能解释并验证框架映射：已有 LangGraph 能力 → Deep Agents abstraction"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：理解 Deep Agent 为什么需要 planning/subagents/context/workspace"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：已有 LangGraph 能力 → Deep Agents abstraction"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：过度自治"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
