...
- 学习 relevance/recency/importance
- 设计写入策略
...
- always write → selective write
- global store → tenant/user namespace
...
````yaml
chapter_id: stage-06-02-memory-engineering
prerequisites: ["stage-06-01-memory-model"]
required_exercises:
  - id: EX1
    task: "围绕Memory 工程：写入、检索、遗忘与隔离，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：学习 relevance/recency/importance"
      - "能复现或具体分析工程问题：记忆无限增长"
      - "能解释并验证框架映射：always write → selective write"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：学习 relevance/recency/importance"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：always write → selective write"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：记忆无限增长"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
