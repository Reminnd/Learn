# 用户自选项目综合集成 / Chapter 01

# 用户自选项目：架构设计与里程碑

## 本章定位

本章属于 `stage-14`。只有在用户已经选择长期项目并启用 Project Track 后才正式进入；否则先协助用户确定项目。
...
- 各章节 Demo → unified architecture
- framework features → harness boundaries
...
````yaml
chapter_id: stage-14-01-capstone-architecture
prerequisites: ["stage-13-03-docker-kubernetes"]
required_exercises:
  - id: EX1
    task: "围绕用户自选项目：架构设计与里程碑，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：整合所有能力"
      - "能复现或具体分析工程问题：过度设计"
      - "能解释并验证框架映射：各章节 Demo → unified architecture"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：整合所有能力"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：各章节 Demo → unified architecture"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：过度设计"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
