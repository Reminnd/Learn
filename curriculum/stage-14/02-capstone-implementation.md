# 用户自选项目综合集成 / Chapter 02

# 用户自选项目：实现、测试与验收

## 本章定位

本章属于 `stage-14`。只有在用户已经选择长期项目并启用 Project Track 后才正式进入。
...
- isolated demos → end-to-end system
- manual test → automated eval/trace
...
````yaml
chapter_id: stage-14-02-capstone-implementation
prerequisites: ["stage-14-01-capstone-architecture"]
required_exercises:
  - id: EX1
    task: "围绕用户自选项目：实现、测试与验收，实现最小原理版并用框架表达同一能力"
    acceptance:
      - "产出可运行代码或可执行伪代码，展示：按里程碑集成"
      - "能复现或具体分析工程问题：模块集成失败"
      - "能解释并验证框架映射：isolated demos → end-to-end system"
questions:
  - id: Q1
    prompt: "为什么需要这项能力：按里程碑集成"
    critical: true
    acceptance:
      - "给出核心因果关系，而不是只复述定义"
      - "说明至少一个适用边界或失败条件"
  - id: Q2
    prompt: "如何验证这项原理—框架映射：isolated demos → end-to-end system"
    critical: true
    acceptance:
      - "明确原理侧与框架侧的对应对象"
      - "说清输入、输出与关键数据流"
  - id: Q3
    prompt: "遇到该工程问题时如何定位并权衡方案：模块集成失败"
    critical: false
    acceptance:
      - "给出可执行的排查顺序"
      - "说明至少一个方案 Trade-off"
